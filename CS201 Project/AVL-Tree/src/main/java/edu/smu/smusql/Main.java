...
public class Main {
    ...
    public class DatabaseAutoEvaluator {
        ...
        private static final int NUMBER_OF_QUERIES = 10000000;
        ...
        private static void executeSelectQuerySimple() {
            int targetId = RANDOM.nextInt(userCount);
            String query = "SELECT * FROM users WHERE id = " + targetId;
            ...
            double queryTime = (queryEnd - queryStart) / 1_000_000.0;
            
            if (totalTime > 10.0) { // Log slow queries
                System.out.printf("Slow SELECT: Total=%.2fms, Query=%.2fms, ID=%d%n", 
                    totalTime, queryTime, targetId);
            }
        }
